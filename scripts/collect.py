...
def fetch_pdf(date: str, ticker: str) -> pd.DataFrame | None:
    ...
    if raw is None or raw.empty:
        # 예비 경로: pykrx 기본 API (컬럼명이 다르므로 맞춰준다)
        try:
        ...
        if df is None or df.empty:
            return None
        df = df.reset_index().rename(columns={"금액": "평가금액", "시가총액": "구성금액"})
    ...
    # 비중은 '-' 로 오는 경우가 많다. 그런 값은 0으로 두고 나중에 구성금액에서 환산한다.
    df = df.replace(",", "", regex=True).replace(r"^\s*-\s*$", "0", regex=True)
    for col, kind in (("계약수", np.float64), ("평가금액", np.int64),
                      ("구성금액", np.int64), ("비중", np.float64)):
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0).astype(kind)

    df = df[(df[["계약수", "평가금액", "구성금액", "비중"]] != 0).any(axis=1)]
